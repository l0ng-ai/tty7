...
- **Agent coordination** — a switch (Settings → Agents, also offered once the first time tty7 sees an agent running) that installs a short marked note describing the session CLI into `~/.claude/CLAUDE.md` (and `~/.codex/AGENTS.md` when present), so agents know the commands exist; disabling removes exactly what tty7 wrote and nothing else
