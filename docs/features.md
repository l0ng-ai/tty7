...
- **Rearrange splits by dragging** — hover a pane and a small grip appears along its top edge; drag it over the layout to put the pane somewhere else in the tab. Dropping on a pane's edge splits that pane and takes that side of it, dropping on its middle trades the two panes' places, and dropping in the band along the outside of the tab makes the pane a full-width or full-height band beside everything else — so a pane in the middle of a 2×2 becomes a full-height column in one drag. The landing lights up while you drag, and only ever lights up when the drop would really change the layout
