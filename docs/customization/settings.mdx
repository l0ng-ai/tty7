...
<CardGroup cols={2}>
  ...
  <Card title="Window & Tabs" icon="window-maximize">
    Tab position and placement, sidebar auto grouping, diff previews, and what
    SSH tabs are called.
  </Card>
...
The one exception is your saved SSH hosts, which live beside it in
`servers.json` — so `config.json` can be synced between machines without
carrying your servers. [servers.json →](/reference/configuration#ssh)
