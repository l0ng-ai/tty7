...
<CardGroup cols={2}>
  <Card title="General" icon="sliders">
    Interface language, default terminal, startup window, size and layout
    restore, tray icon, and command completion notifications.
  </Card>
  ...
  <Card title="SSH" icon="server" href="/remote/ssh">
    Recent and saved hosts, each opened and edited in place, and host key
    security.
  </Card>
  ...
  <Card title="About" icon="circle-info" href="/reference/updates">
    Version and update checks, update preferences and proxy, the settings
    file, and the tty7 server.
  </Card>
</CardGroup>
...
keys such as `mouse_zoom_modifier`. Results are grouped by section and are
editable directly; click a section name to open it. Press ⌘F (Ctrl+F) to jump
to the search box.

Use **Modified only** to find settings that differ from their defaults. A
modified setting offers **Reset** beside its name; installation and maintenance actions
...
cancel restores the original theme. SSH hosts also use explicit saving.

Recording a keyboard shortcut that another action already uses asks before
taking it over: **Replace** moves the shortcut, **Cancel** leaves both as they
were.
