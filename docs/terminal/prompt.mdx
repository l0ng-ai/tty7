...
Each prompt feature is a switch, and turning one off hands its key straight
back to the shell:
...
To hand the whole prompt back, turn off **Settings → Input → Prompt → Prompt
editor** (`prompt_editor` in `config.json`). Every key, click, and paste then
goes straight to your shell's own line editor — <kbd>↑</kbd> runs the ZLE
widget or readline binding you put on it, not tty7's history. Shell integration
stays on: prompt marks, working-directory tracking, and the command-finished
