...
`{path}`, `{line}`, and `{column}` are filled in from the link. A value the
link does not carry is dropped rather than passed empty, along with whatever
introduced it: `--line={line}` disappears entirely, and `{path}:{line}` on a
link with no line number becomes just the path, with no trailing colon.
