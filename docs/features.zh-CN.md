...
- **Agent coordination** —— 一个开关（设置 → Agents；tty7 第一次检测到 agent 运行时也会主动询问一次），往 `~/.claude/CLAUDE.md`（存在 `~/.codex` 时还有 `~/.codex/AGENTS.md`）写入一段带标记的 session CLI 简短说明，让 agent 知道这些命令存在；关闭时只移除 tty7 写入的那一段
