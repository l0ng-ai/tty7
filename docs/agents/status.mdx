...
| Agent | |
|---|---|
| Claude Code · Codex · TraeCode · Copilot CLI · OpenCode · Pi · Grok Build · Oh My Pi · Gemini · Droid · Qwen Code · Goose · Kimi Code · Qoder CLI · Crush · CodeBuddy · Cursor CLI · Prime Agent · Antigravity | Hooks available |
| Aider · Amp · Auggie · Hermes · Vibe · Empryo | Detected and labelled, but no status channel yet |
...
<Note>
  Antigravity has no session-start or permission event, so its pane goes
  straight from no dot to working on the first model call, and a pending
  permission prompt shows as working rather than amber. Its hooks live in a
  `tty7` set inside `~/.gemini/config/hooks.json`, beside any sets of your own.
</Note>
