...
Twenty-five CLIs are recognised on sight, by the command running in the pane:
...
`omp`, `kimi`, `qodercli`, `crush`, `codebuddy`, `empryo`, `prime-agent`).

## Quick launch

Every agent tty7 finds on your `PATH` is one palette command away: **Agent:
Claude Code**, **Agent: Codex**, and so on, most-used first. The New Tab menu's
**Launch Agent…** row opens the palette on that list, and **New Agent Tab**
