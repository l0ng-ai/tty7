...
An SSH tab is named by whatever the remote shell titles itself, like any other.
To keep it on the host instead, set **Settings → Window & Tabs → SSH tab
title** to *Profile name* — the saved host's name, the `~/.ssh/config` alias, or
the address you typed — or *Hostname*. A tab you renamed keeps its name either
way, and a split tab still follows the pane in front.
...
Keybindings**, or run them from Search Everywhere (*Resize Pane Left*, *Swap
