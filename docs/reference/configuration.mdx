...
`command_frecency` records how often and how recently you run a command, so the
palette can rank it. It is written by the app; there is no reason to edit it.
