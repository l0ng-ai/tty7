...
`ssh_profile_frecency`, `shell_frecency` and `command_frecency` record how often
and how recently you use a profile, shell or command, so the pickers and the
new-tab menu can rank them. They are written by
