...
| `sidebar_width` | number | `220` | Pixels (180–2000). Also capped to the window. |
| `sidebar_collapsed` | bool | `false` | |
| `right_panel_visible` | bool | `false` | |
| `right_panel_width` | number | `260` | Pixels (216–2000). Also capped to the window. |
...
| `prefix` | string | `"ctrl-b"` | The tmux preset's prefix. One chord carrying a real modifier — anything else falls back to the default. |
...
| `link_file_command` | string | — | Command for file links under `link_file_open: command`. `{path}`, `{line}`, `{column}` are substituted; a missing value is dropped with whatever introduced it, keeping the path. |
...
| Key | Type | Default | |
|---|---|---|---|
| `agent_commands` | object | `{}` | Map a wrapper command to an agent slug: `{"cc": "claude"}`. Read once per server, so a new entry needs `tty7 server restart`. |
