...
pub fn translate_en(key: L10nKey) -> &'static str {
    match key {
        ...
        L10nKey::Trust => "Trust",
        L10nKey::Abort => "Abort",
        L10nKey::HostKeyOverrideMessage => "Type \"yes\" to override and trust the new key, or Esc to abort.",
        L10nKey::Override => "Override",
        L10nKey::RememberKeychain => "Remember (keychain)",
        L10nKey::CloseWindowTitle => "Close Window?",
        L10nKey::CloseWindowBody => "Your sessions keep running in the background. This workspace will be \
        ...
        L10nKey::QuitStopServerBody => "This quits tty7 and stops the background server — anything still running \
        ...
        L10nKey::SettingsThemeIntroDesc => "Pick a color theme. Each one sets its own light or dark look.",
        ...
        L10nKey::SettingsBoldFontDesc => "Face for bold text; Default synthesizes it from the primary.",
        L10nKey::SettingsItalicFont => "Italic font",
        L10nKey::SettingsItalicFontDesc => "Face for italic text; Default synthesizes it from the primary.",
        L10nKey::SettingsFontLigatures => "Font ligatures",
        L10nKey::SettingsFontLigaturesDesc => "Enable common programming ligature features for terminal text.",
        ...
        L10nKey::SettingsOpacityDesc => "How opaque the window background is, for every theme. Below 100% the desktop shows through.",
        ...
        L10nKey::SettingsDimInactivePanesDesc => "Fade unfocused panes in a split so the active one stands out.",
        ...
        L10nKey::SettingsImageOpacityDesc => "How strongly the image shows over the background color.",
        L10nKey::SettingsEditTheme => "Edit theme",
        L10nKey::SettingsEditThemeIntro => "You're editing a copy. Changes save to its file in the themes folder and apply live.",
        L10nKey::SettingsBackgroundImage => "Background image",
        L10nKey::SettingsBackgroundImageDesc => "Composited over the background color, under the text.",
        ...
        L10nKey::SettingsTypeAddressToConnect => "Type an address to connect now — tty7 offers to save it afterwards.",
        ...
        L10nKey::SettingsImportAliasesDesc => "Re-reads the file and adds anything new. Edits you make here are stored by tty7 — the file itself is never written.",
        L10nKey::SettingsImportNow => "Import now",
        L10nKey::SettingsDefaultsIntro => "Every host starts from these. Any host can override one under its own Advanced.",
        ...
        L10nKey::SettingsCouldntForgetPassword => "Couldn't forget password for {endpoint}: {error}",
        L10nKey::SettingsSecurity => "Security",
        L10nKey::SettingsSecurityIntro => "A host can override either of these under its own Advanced.",
        L10nKey::SettingsVerifyHostKeys => "Verify host keys",
        L10nKey::SettingsVerifyHostKeysDesc => "Check each server's key against known_hosts and confirm unknown or changed keys before connecting. Off connects without checking, so a spoofed server would go unnoticed.",
        L10nKey::WarnBeforeClosing => "Warn before closing",
        L10nKey::SettingsWarnBeforeClosingDesc => "Ask for confirmation before closing a tab or pane with a live SSH session.",
        ...
        L10nKey::SettingsJumpHostDesc => "Name of another profile to tunnel through (blank = direct).",
        ...
        L10nKey::SettingsFwdNeedsBoth => "Needs a listen port and a target host:port — won't be saved.",
        L10nKey::SettingsFwdNeedsListen => "Needs a listen port — won't be saved.",
        L10nKey::SettingsAdvanced => "Advanced",
        L10nKey::SettingsAdvancedSummary => "algorithms / keepalive / proxies / X11 / login scripts",
        ...
        L10nKey::SettingsShellIntegrationDesc => "Let the remote shell report prompts, exit codes and directory.",
        ...
        L10nKey::SettingsShellIntro => "The program each new terminal launches. Leave Program empty to use the platform default ({default}).",
        L10nKey::SettingsProgram => "Program",
        L10nKey::SettingsProgramDesc => "Executable name on PATH or an absolute path. e.g. zsh, fish, pwsh.",
        L10nKey::SettingsArguments => "Arguments",
        L10nKey::SettingsArgumentsDesc => "Space-separated launch flags. e.g. -l for a login shell.",
        L10nKey::SettingsStartIn => "Start in",
        L10nKey::SettingsStartInDesc => "What a fresh shell starts in: tty7's launch directory, your home folder, or a fixed path.",
        ...
        L10nKey::SettingsShellFooter => "Applies to shells with nothing to inherit — like the first tab of a window. New tabs and splits keep inheriting the active pane's directory, and shells already open keep running.",
        ...
        L10nKey::SettingsHideMouseWhileTypingDesc => "Hide the pointer as you type; it returns on the next move.",
        L10nKey::SettingsReportMouseToApps => "Report mouse to apps",
        L10nKey::SettingsReportMouseToAppsDesc => "Let full-screen apps (vim, tmux) handle clicks and scrolling; hold Shift to keep a gesture local.",
        L10nKey::SettingsBell => "Bell",
        L10nKey::SettingsTerminalBell => "Terminal bell",
        L10nKey::SettingsTerminalBellDesc => "How a bell (^G) is signalled: silenced, a brief flash, or the system sound.",
        L10nKey::SettingsLinks => "Links",
        L10nKey::DetectUrls => "Detect URLs",
        L10nKey::SettingsDetectUrlsDesc => "Underline links on hover and open them on {modifier}-click.",
        L10nKey::ForwardSshLoopbackLinks => "Forward SSH loopback links",
        L10nKey::SettingsForwardSshLoopbackLinksDesc => "When a pane is in SSH, open localhost links through a temporary port forward.",
        L10nKey::OpenFilesWith => "Open files with",
        L10nKey::SettingsOpenFilesWithDesc => "Command run when {modifier}-clicking a file link, instead of the default app. Use {path}, {line}, {column}; a flag whose value is absent is dropped (e.g. herdr edit {path} --line={line}). Empty uses the default app.",
        ...
        L10nKey::SettingsPromptIntro => "tty7's own menus at the shell prompt. Turn one off to hand the key back to the shell.",
        L10nKey::SettingsTabCompletion => "Tab completion",
        L10nKey::SettingsTabCompletionDesc => "Tab at the prompt opens tty7's completion menu. When off, Tab goes to the shell's own completion instead.",
        L10nKey::SettingsHistorySearch => "History search",
        L10nKey::SettingsHistorySearchDesc => "⌃R at the prompt opens tty7's fuzzy history menu. When off, ⌃R goes to the shell instead — its own reverse-i-search, or whatever you've bound there (fzf, percol).",
        L10nKey::SettingsSelectionClipboard => "Selection & clipboard",
        L10nKey::SettingsSmartSelection => "Smart selection",
        L10nKey::SettingsSmartSelectionDesc => "Double-click selects the whole URL, file path, email, or bracket pair under the cursor.",
        L10nKey::SettingsCopyOnSelect => "Copy on select",
        L10nKey::SettingsCopyOnSelectDesc => "Selecting text with the mouse copies it to the clipboard right away, no ⌘C needed.",
        L10nKey::SettingsTrimTrailingSpaces => "Trim trailing spaces on copy",
        L10nKey::SettingsTrimTrailingSpacesDesc => "Strip trailing whitespace from each copied line.",
        L10nKey::SettingsKeyboard => "Keyboard",
        L10nKey::SettingsOptionAsMeta => "Option (⌥) acts as Meta",
        L10nKey::SettingsOptionAsMetaDesc => "⌥+key sends the escape chord shells expect (⌥B = back one word) instead of typing a special character (∫).",
        L10nKey::SettingsAgentsIntro => "Agents",
        L10nKey::SettingsAgentsIntroDesc => "Hook integrations give panes running these agents live session status (working / waiting / done) in the tab bar. Only active inside tty7.",
        ...
        L10nKey::SettingsOfflineMachines => "{count} more saved machines are not connected — open a workspace on one to install its hooks there.",
        L10nKey::SettingsSyncWithSystem => "Sync with system",
        L10nKey::SettingsSyncWithSystemDesc => "Follow the OS appearance with separate light and dark themes.",
        ...
        L10nKey::SettingsRememberWindowSizeDesc => "Reopen at the size and position the window had when tty7 last quit. Off opens centered at the default size.",
        L10nKey::SettingsRestoreLastLayout => "Restore last layout",
        L10nKey::SettingsRestoreLastLayoutDesc => "Reopen the last window's tabs, splits, and directories on launch. Off starts with a single fresh terminal.",
        L10nKey::SettingsConfirmLastWindowClose => "Confirm before closing the last window",
        L10nKey::SettingsConfirmLastWindowCloseDesc => "Ask first, since that close also quits tty7. Off closes straight away — either way your shells keep running in the background.",
        L10nKey::SettingsShowTrayIcon => "Show tray icon",
        L10nKey::SettingsShowTrayIconDesc => "Keep a status item in the system tray / menu bar: it signals when a coding agent needs your input, and its menu jumps to agent panes.",
        ...
        L10nKey::SettingsTabBarPositionDesc => "Show tabs as a horizontal strip on top or a vertical sidebar on the left.",
        L10nKey::SettingsSidebarGrouping => "Sidebar grouping",
        L10nKey::SettingsSidebarGroupingDesc => "Group sidebar tabs under a header per git repository, with non-repo tabs in a Scratch section. Only applies to the left sidebar.",
        L10nKey::SettingsDiffPreviewFromCounts => "Open diff preview from sidebar counts",
        L10nKey::SettingsDiffPreviewFromCountsDesc => "Click a row's +N −N to open the working-tree diff in an overlay. Off keeps the branch and the counts on the row and just stops them being clickable.",
        L10nKey::SettingsNotifications => "Notifications",
        L10nKey::SettingsNotifyOnCommandFinish => "Notify on command finish",
        L10nKey::SettingsNotifyOnCommandFinishDesc => "Desktop alert after a long foreground command completes.",
        L10nKey::SettingsNotifyThreshold => "Notify threshold",
        L10nKey::SettingsNotifyThresholdDesc => "How long a command must run to qualify as \"long\".",
        ...
        L10nKey::SettingsPresetDesc => "tmux remaps pane/tab actions onto prefix sequences (e.g. Ctrl-B then C).",
        L10nKey::SettingsPrefix => "Prefix",
        L10nKey::SettingsPressKeys => "Press keys…",
        L10nKey::SettingsPauseToSaveEsc => "pause to save · Esc",
        L10nKey::SettingsKeybindingsIntroDesc => "Click a shortcut, then press the new keys — it saves after a brief pause. Chain keys for a sequence like Ctrl-B then X. Esc cancels; Backspace removes the last key, or resets the shortcut to default when pressed first.",
        L10nKey::SettingsPrefixNote => "With a prefix active, a bare prefix key reaches the shell after a ~1s pause, and prefix + an unbound key is sent through to the terminal.",
        L10nKey::SettingsRestoreAllDefaults => "Restore all defaults",
        L10nKey::SettingsAboutDesc1 => "A terminal workbench: shells, workspaces, SSH, coding agents.",
        L10nKey::SettingsAboutDesc2 => "Editor-grade input in every shell, shells that outlive quits and reboots without tmux, a native SSH stack with profiles and port forwarding, and live status for panes running coding agents.",
        L10nKey::SettingsAboutTech => "Pure Rust · GPU rendering on Zed's gpui · VT core from Alacritty",
        ...
        L10nKey::SettingsCheckUpdatesDesc => "Check GitHub for a newer release on launch and show it here. tty7 never updates itself — downloading happens on the Releases page.",
        L10nKey::SettingsCheckUpdatesOnLaunch => "Check for updates on launch",
        L10nKey::SettingsCommandLine => "Command line",
        L10nKey::SettingsCommandLineDesc => "Put the bundled `tty7` command on your PATH at launch, so scripts and coding agents can drive tty7 from any terminal. Inside a tty7 pane it works either way. Turn this off if you keep your own `tty7` — one you built or installed yourself — and do not want it shadowed. Takes effect at next launch.",
        L10nKey::SettingsInstallCliOnPath => "Install the `tty7` command on PATH",
        L10nKey::SettingsExplorerContextMenu => "Windows Explorer",
        L10nKey::SettingsExplorerContextMenuDesc => "Add “Open in tty7” when you right-click a folder and “Open tty7 here” when you right-click a folder background. This is off by default and is registered only for your Windows account.",
        L10nKey::SettingsExplorerNotRegistered => "Not registered",
        L10nKey::SettingsExplorerRegistered => "Registered",
        L10nKey::SettingsExplorerNeedsUpdate => "Needs update",
        L10nKey::SettingsExplorerUnavailable => "Unavailable",
        L10nKey::SettingsExplorerStatusUnavailable => "Status unavailable",
        L10nKey::SettingsExplorerRegister => "Register",
        L10nKey::SettingsExplorerUpdate => "Update",
        L10nKey::SettingsExplorerUnregister => "Unregister",
        L10nKey::SettingsExplorerRegisteredNote => "Registered. Right-click a folder or folder background in Explorer to open it in tty7.",
        L10nKey::SettingsExplorerUnregisteredNote => "Unregistered from Windows Explorer.",
        L10nKey::SettingsExplorerRegisterFailed => "Could not register: {error}",
        L10nKey::SettingsExplorerUnregisterFailed => "Could not unregister: {error}",
        L10nKey::SettingsExplorerWindows11Note => "On Windows 11, classic shell entries may appear under “Show more options”.",
        L10nKey::SettingsServer => "Server",
        L10nKey::SettingsServerDesc => "Restart the server on this computer to pick up a newly granted macOS permission, recover if it stops responding, or start from a clean slate. This ends all running shells here; your tabs and layout reopen with fresh shells. A remote machine's server is restarted from its own menu in the workspace switcher.",
        ...
        L10nKey::SettingsSearchBlurKeywords => "transparency translucent frosted vibrancy window background",
        L10nKey::SettingsSearchBoldFontKeywords => "typeface weight",
        L10nKey::SettingsSearchClaudeCodeKeywords => "agent integration hooks install uninstall status rich session working waiting tab bar sidebar badge claude",
        L10nKey::SettingsSearchCodexKeywords => "agent integration hooks install openai codex",
        L10nKey::SettingsSearchCommandLineToolKeywords => "cli tty7 path shell command install symlink terminal iterm agent script",
        L10nKey::SettingsSearchCommandLineToolTitle => "Command line tool",
        L10nKey::SettingsSearchConfirmLastWindowCloseKeywords => "close quit confirm prompt dialog ask again warn last window cmd-w ctrl-w",
        L10nKey::SettingsSearchCopilotCliKeywords => "agent integration hooks install github copilot",
        ...
        L10nKey::SettingsSearchCustomThemesKeywords => "theme duplicate edit colors folder yaml import",
        L10nKey::SettingsSearchDetectUrlsKeywords => "links hyperlink clickable open",
        L10nKey::SettingsSearchDiffPreviewFromCountsKeywords => "diff overlay preview sidebar counts git changes click branch lines",
        L10nKey::SettingsSearchDimInactivePanesKeywords => "fade unfocused inactive split pane focus opacity highlight active dimming",
        L10nKey::SettingsSearchExplorerContextMenuKeywords => "windows explorer context menu right click folder directory background shell menu register unregister open here",
        ...
        L10nKey::SettingsSearchForwardSshLoopbackLinksKeywords => "ssh remote port tunnel localhost forward links",
        L10nKey::SettingsSearchGrokBuildKeywords => "agent integration hooks install xai grok build",
        L10nKey::SettingsSearchHideMouseWhileTypingKeywords => "cursor pointer autohide",
        L10nKey::SettingsSearchHistorySearchKeywords => "ctrl-r reverse search fuzzy history recall fzf prompt",
        L10nKey::SettingsSearchHostsKeywords => "ssh host connection saved profile import ssh_config manage add edit quick connect",
        L10nKey::SettingsSearchHowShellsWorkKeywords => "shell session daemon server detach persist background close quit stop delete workspace layout survive reboot tmux",
        L10nKey::SettingsSearchHowShellsWorkTitle => "How shells work",
        L10nKey::SettingsSearchItalicFontKeywords => "typeface oblique",
        L10nKey::SettingsSearchKeybindingsKeywords => "shortcut hotkey keyboard binding chord tmux preset rebind prefix",
        ...
        L10nKey::SettingsSearchNotifyOnCommandFinishKeywords => "notification alert done osc desktop banner long command",
        L10nKey::SettingsSearchNotifyThresholdKeywords => "notification alert seconds duration long command delay",
        L10nKey::SettingsSearchOpacityKeywords => "transparency translucent see through window alpha",
        L10nKey::SettingsSearchOpenFilesWithKeywords => "links file editor command external app path line column",
        L10nKey::SettingsSearchOpencodeKeywords => "agent integration plugin install opencode",
        L10nKey::SettingsSearchOptionAsMetaKeywords => "alt keyboard modifier escape macos option meta option acts as meta",
        L10nKey::SettingsSearchPiKeywords => "agent integration extension install pi",
        L10nKey::SettingsSearchPortForwardingKeywords => "ssh tunnel local remote dynamic socks forward rule",
        L10nKey::SettingsSearchProgramKeywords => "shell binary zsh bash fish nu nushell pwsh powershell executable launch",
        L10nKey::SettingsSearchRememberWindowSizeKeywords => "window size position bounds geometry launch startup remember",
        L10nKey::SettingsSearchReportMouseToAppsKeywords => "mouse reporting vim tmux click scroll shift passthrough",
        L10nKey::SettingsSearchRestoreLastLayoutKeywords => "restore session previous tabs splits reopen launch startup layout",
        ...
        L10nKey::SettingsSearchShowTrayIconKeywords => "tray menu bar status item agent attention system icon",
        L10nKey::SettingsSearchSidebarGroupingKeywords => "tabs group repo repository git scratch header sidebar flat",
        L10nKey::SettingsSearchSmartSelectionKeywords => "double click word url path select semantic bracket email",
        L10nKey::SettingsSearchStartInKeywords => "cwd working directory start folder path home inherit custom",
        L10nKey::SettingsSearchSyncWithSystemKeywords => "theme dark light auto follow os appearance mode",
        L10nKey::SettingsSearchTabBarPositionKeywords => "tabs vertical sidebar left top layout rail",
        L10nKey::SettingsSearchTabCompletionKeywords => "complete completion menu suggestions tab prompt",
        L10nKey::SettingsSearchTerminalBellKeywords => "bell audible visual flash sound silence beep ^g",
        L10nKey::SettingsSearchThemeKeywords => "appearance color colours scheme dark light palette background foreground accent sync system os auto follow",
        L10nKey::SettingsSearchTrimTrailingSpacesKeywords => "clipboard whitespace copy",
        L10nKey::SettingsSearchVerifyHostKeysKeywords => "ssh security known_hosts fingerprint mitm host key verification",
        L10nKey::SettingsSearchWarnBeforeClosingKeywords => "ssh confirm close tab pane live session security",
        ...
        L10nKey::PanelNoSessionHint => "Open a tab to see its shell, directory, and processes here.",
        L10nKey::PanelNoCommands => "No commands recorded for this pane.",
        L10nKey::PanelNoCommandsHint => "Run a command — shell integration marks each one so you can jump back to it.",
        ...
        L10nKey::WindowStopUnreachable => "Its machine could not be reached. Any shells still running there will be ended.",
        L10nKey::WindowDeleteUnreachable => "Its machine could not be reached. Any shells still running there will be ended, and the layout forgotten.",
        L10nKey::WindowStopShells => "{count} running shells will be ended.",
        L10nKey::WindowDeleteShells => "{count} running shells will be ended and the layout forgotten.",
        L10nKey::DiffReading => "Reading diff…",
        L10nKey::DiffNotARepo => "Not a git repository",
        L10nKey::DiffReadFailed => "Couldn't read the working-tree diff — retrying on the next refresh.",
        ...
        L10nKey::DiffMoreFiles => "… and {count} more changed files — run `git diff` in the terminal to see them.",
        L10nKey::DiffOversizedNotice => "This working tree is too large to render efficiently ({summary}). Every file is collapsed — expand individual files, or run `git diff` in the terminal.",
        L10nKey::DiffTruncatedPerFile => "Diff truncated at {limit} lines — run `git diff` in the terminal for the rest.",
        L10nKey::DiffTruncatedBudget => "Body not loaded — this working tree is past tty7's diff budget. Run `git diff` in the terminal for this file.",
        L10nKey::DiffUntrackedHeader => "Untracked files ({count})",
        L10nKey::DiffMoreUntracked => "… and {count} more — run `git status` in the terminal to see them.",
        L10nKey::DiffLines => "{count} diff lines",
        L10nKey::DiffChangedLines => "{total} changed lines, {loaded} diff rows loaded before {cap} cut the rest",
        ...
        L10nKey::RemoteNoConnectionDetails => "This window is a workspace on {machine}, but tty7 has no connection \
        ...
        L10nKey::RemoteRestartBody => "This stops every shell on {machine} — anything still running in them \
        ...
        L10nKey::RemoteReplaceBody => "The tty7-server running on {machine} speaks a protocol this client \
        ...
        L10nKey::RemoteInstallDetail => "tty7 will write its server binary to {machine} so this machine can host \
        ...
        L10nKey::RemoteMismatchTitle => "Restart tty7's server on \"{machine}\"?",
        L10nKey::RemoteMismatchDetail => "{machine} is serving tty7 sessions from {running}, which speaks a protocol \
        ...
        L10nKey::RemoteDaemonTooOld => "this machine's tty7 daemon is an older build and cannot restart the server on \
        ...
        L10nKey::RemoteWorkspaceListFailed => "connected to {machine}, but its workspace list failed: {error}",
        L10nKey::RemoteServerRestartFailed => "could not restart tty7's server on {machine}: {error}",
        L10nKey::RemoteNoRouteToHost => "tty7 no longer has a way to reach {machine}",
        L10nKey::RemoteMachineTreeUnexpectedReply => "the server answered a machine tree with {reply}",
        L10nKey::RemoteMismatchVersionFromExe => "{version} (from {exe})",
        L10nKey::AppNoRunningCodingAgent => "No running coding agent found — start one (claude, codex, …) in a pane first.",
        L10nKey::SwitcherThisComputer => "This Computer",
        L10nKey::SwitcherRestartingServer => "Restarting tty7's server…",
        L10nKey::SwitcherDownloadingServerWithTotal => "Downloading tty7's server… {done} / {total}",
        ...
        L10nKey::SshPromptHostKeyChangedBody => "The host key differs from the one previously trusted. This may be an attack.",
        ...
        L10nKey::AppRestartServerOldDetail => "The server holding your shells is from an older version of the app. You can keep using it and your shells stay, but newer features may misbehave until it's restarted. Restarting starts a clean server: tabs reopen with fresh shells and anything running in them is terminated.",
        L10nKey::AppKeepShells => "Keep Shells",
        L10nKey::AppRestart => "Restart",
        L10nKey::AppRestartServerNotSsh => "tty7 can only restart the server on machines it reaches over SSH. {label} is served from this computer — stop its workspace instead.",
        L10nKey::AppRestartServerBody => "This stops every running shell on this computer — anything still running in them will be terminated. Your tabs and layout are kept and reopened with fresh shells.",
        L10nKey::AppWorktreeRemoveDetailDirty => "The closed tab's worktree at {path} has uncommitted changes.",
        ...
        L10nKey::AppForkNoSessionId => "tty7 hasn't seen a {name} session id in this pane — install its hooks in Settings → Agents",
        ...
        L10nKey::AppNoUncommittedChanges => "No uncommitted changes in {cwd} (or not a git repository).",
        ...
        L10nKey::AppAgentHooksNoHomeDir => "tty7 could not work out this computer's home directory, so there is nowhere to install to.",
        L10nKey::AppAgentHooksOffline => "Not connected to this machine, so its agent config can't be read or written. Open a workspace on it and come back.",
        L10nKey::AppAgentHooksHomeDirUnresolved => "cannot resolve home directory",
        L10nKey::AppAgentHooksOpFailed => "Failed: {error}",
        L10nKey::AppKeybindingDisplacedNote => "{action} took the shortcut from {previous}, which is now unset.",
        ...
        L10nKey::PanelMoreChangedFiles => "… and {count} more changed files — run `git diff` to see them.",
...
pub fn translate_variant_en(key: L10nKey, branch: &'static str) -> Option<&'static str> {
    let res = match (key, branch) {
        ...
        (L10nKey::SettingsRulesOpenedWithConnection, "zero") => "0 rules, opened with the connection",
        (L10nKey::SettingsRulesOpenedWithConnection, "one") => "1 rule, opened with the connection",
        (L10nKey::SettingsRulesOpenedWithConnection, "other") => "{count} rules, opened with the connection",
        (L10nKey::SettingsOfflineMachines, "zero") => "0 more saved machines are not connected — open a workspace on one to install its hooks there.",
        (L10nKey::SettingsOfflineMachines, "one") => "1 more saved machine is not connected — open a workspace on it to install its hooks there.",
        (L10nKey::SettingsOfflineMachines, "other") => "{count} more saved machines are not connected — open a workspace on one to install its hooks there.",
        ...
        (L10nKey::PanelMoreChangedFiles, "zero") => "… and 0 more changed files — run `git diff` to see them.",
        (L10nKey::PanelMoreChangedFiles, "one") => "… and 1 more changed file — run `git diff` to see it.",
        (L10nKey::PanelMoreChangedFiles, "other") => "… and {count} more changed files — run `git diff` to see them.",
        ...
        (L10nKey::DiffMoreFiles, "zero") => "… and 0 more changed files — run `git diff` in the terminal to see them.",
        (L10nKey::DiffMoreFiles, "one") => "… and 1 more changed file — run `git diff` in the terminal to see it.",
        (L10nKey::DiffMoreFiles, "other") => "… and {count} more changed files — run `git diff` in the terminal to see them.",
        ...
        (L10nKey::DiffMoreUntracked, "zero") => "… and 0 more — run `git status` in the terminal to see them.",
        (L10nKey::DiffMoreUntracked, "one") => "… and 1 more — run `git status` in the terminal to see it.",
        (L10nKey::DiffMoreUntracked, "other") => "… and {count} more — run `git status` in the terminal to see them.",
        ...
        (L10nKey::WindowStopShells, "zero") => "Its layout and working directories will be forgotten.",
        ...
        (L10nKey::WindowDeleteShells, "zero") => "Its layout and working directories will be forgotten.",
        (L10nKey::WindowDeleteShells, "one") => "1 running shell will be ended and its layout forgotten.",
        (L10nKey::WindowDeleteShells, "other") => "{count} running shells will be ended and the layout forgotten.",
        _ => return None,
    };
    Some(res)
}
