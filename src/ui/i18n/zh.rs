...
pub fn translate_zh(key: L10nKey) -> Option<&'static str> {
    Some(match key {
        ...
        L10nKey::RemoteDaemonTooOld => {
            "本机的 tty7 server 版本较旧，无法重启 {machine} 上的 server。请退出 tty7（这会停止本机的 server）再打开，然后重试。"
        }
        ...
        L10nKey::SettingsDaemonStaleDescInPlace => {
            "tty7 是原地更新的：应用是新的，窗格还跑在旧版上。server 可以不停机就换成新版，shell 直接延续下来。用 tty7 内置 SSH 客户端的窗格除外——那些连接会断开，需要重新打开。"
        }
        L10nKey::AppRestartServerBodyInPlace => {
            "后台 server 会原地把自己换成当前这个版本：shell 继续运行，窗口稍后自动连回去。用 tty7 内置 SSH 客户端的窗格除外——那些连接会断开，需要重新打开。"
        }
