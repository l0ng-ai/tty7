...
pub(super) fn url_span_at(text: &str, col: usize) -> Option<(usize, usize, String)> {
    ...
    // Strip trailing punctuation like ).,;'"> so the underline stops where the link
    // does.
    while token
        .chars()
        .next_back()
    ...
    // Strip leading wrappers like ([<'"{ — URLs are often parenthesized or quoted
    // in prose and logs (e.g. `(https://x)`, `[https://x]`, `<https://x>`). Advance
    // `start` per removed char so the reported span still lines up with the trimmed
    // token's cells. These wrappers are ASCII, so `remove(0)` stays on a boundary.
    while token
        .chars()
        .next()
        .is_some_and(|c| matches!(c, '(' | '[' | '<' | '\'' | '"' | '{'))
