...
///
/// `EPERM` is the other way signal 0 answers yes: the process exists but
/// belongs to someone else. Reading only `kill(pid, 0) == 0` calls another
/// user's running process dead, which is the wrong direction for both callers
/// — one decides whether to clean up after a daemon, the other whether to
/// delete a directory it owns. Such a process is not ours to reap and not
/// ours to introspect, so the zombie question does not arise for it.
#[cfg(any(target_os = "macos", target_os = "linux"))]
