...
impl<T> Storage<T> {
    #[cfg(feature = "serde")]
    pub(crate) fn valid_checkpoint(&self, lines: usize, columns: usize, history: usize) -> bool {
        self.visible_lines == lines && self.len >= lines && self.len <= self.inner.len()
            && self.len - lines <= history && self.zero < self.inner.len()
            && self.inner.iter().map(|row| row.len()).sum::<usize>() <= 2_000_000
            && self.inner.iter().all(|row| row.occ <= row.len())
            && (0..self.len).all(|i| self[Line(lines as i32 - 1 - i as i32)].len() == columns)
    }
    #[inline]
