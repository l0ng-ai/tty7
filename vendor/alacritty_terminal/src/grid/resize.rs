...
impl<T: GridCell + Default + PartialEq> Grid<T> {
    ...
    /// Resize the grid with ConPTY's row semantics.
    ///
    /// Windows ConPTY emits no repaint after a resize; conhost silently
    /// re-anchors its viewport and keeps painting with absolute cursor
    /// addresses computed against that layout. A grid that resizes any other
    /// way ends up with its rows offset from every subsequent paint. Conhost's
    /// model, measured against a live ConPTY:
    ///
    /// - Growing the screen adds blank rows at the bottom; nothing is
    ///   restored from scrollback and the cursor row does not move.
    /// - Shrinking scrolls the *last written* row (not just the cursor row —
    ///   e.g. PSReadLine keeps a continuation hint on the row below the
    ///   prompt) to the new bottom, pushing the rows above out into history.
    ...
    ///
    /// With `conpty` set nothing is ever pulled from scrollback: conhost keeps
    /// content and cursor where they are and opens blank rows below, so the
    /// grid must too (see [`Self::resize_conpty`]).
    ...
    ///
    /// With `conpty` set the scroll is anchored to the last *written* row
    /// instead of the cursor row: conhost keeps rows below the cursor (a
    /// PSReadLine continuation hint, a status line) visible when the window
    /// shrinks, so their paints keep landing where conhost aims them. The
    /// cursor is still never scrolled out of the viewport.
        ...
        D: PartialEq,
    {
        ...
        if required_scrolling > 0 {
            ...
            // Move the cursor up with the content it sits on, clamped to the
            // new viewport size.
    ...
    /// Bottommost viewport row holding any written cell, in viewport
    /// coordinates; `0` when the whole screen is blank.
