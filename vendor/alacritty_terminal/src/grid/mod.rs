...
impl<T: GridCell + Default + PartialEq> Grid<T> {
    ...
    pub(crate) fn valid_checkpoint(&self) -> bool {
        self.lines > 0 && self.columns > 0 && self.lines <= 4096 && self.columns <= 4096
            && self.max_scroll_limit <= 100_000
            && self.raw.valid_checkpoint(self.lines, self.columns, self.max_scroll_limit)
            && self.display_offset <= self.raw.len() - self.lines
    }
