...
// ConPTY resize: growing must not restore scrollback — conhost keeps rows and
// cursor pinned and opens blank rows below, and no repaint follows to fix a
// grid that anchored any other way.
#[test]
fn conpty_grow_lines_pins_rows_and_cursor() {
    let mut grid = Grid::<usize>::new(2, 1, 10);
    grid[Line(0)][Column(0)] = 1;
    grid[Line(1)][Column(0)] = 2;
    // Scroll "1" into history, then refill the freed bottom row.
    ...
    assert_eq!(grid.cursor.point.line, Line(1), "cursor row must not move");
    assert_eq!(grid[Line(0)][Column(0)], 2);
    assert_eq!(grid[Line(1)][Column(0)], 3);
    assert_eq!(grid[Line(2)][Column(0)], 0);
    assert_eq!(grid[Line(3)][Column(0)], 0);
    assert_eq!(grid.history_size(), 1, "scrollback must survive untouched");
    assert_eq!(grid[Line(-1)][Column(0)], 1, "with the same content");
}

// ConPTY resize: shrinking anchors to the last written row, not the cursor —
// conhost keeps a row painted below the cursor (PSReadLine continuation hint)
// visible, so the grid must too or later paints land one row off.
#[test]
fn conpty_shrink_lines_keeps_the_last_written_row() {
    let mut grid = Grid::<usize>::new(4, 1, 10);
    for i in 0..4 {
        grid[Line(i)][Column(0)] = i as usize + 1;
    }
    ...
    assert_eq!(grid.cursor.point.line, Line(0), "cursor follows its row");
    assert_eq!(grid.history_size(), 2);
    assert_eq!(grid[Line(-1)][Column(0)], 2);
    assert_eq!(grid[Line(-2)][Column(0)], 1);
}

// ConPTY resize: content anchoring never scrolls the cursor out of the top.
#[test]
fn conpty_shrink_lines_never_scrolls_the_cursor_out() {
    let mut grid = Grid::<usize>::new(4, 1, 10);
    for i in 0..4 {
        grid[Line(i)][Column(0)] = i as usize + 1;
    }
