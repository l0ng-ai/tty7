...
pub struct Config {
    ...
    /// Resize the primary screen with ConPTY's row semantics.
    ///
    /// ConPTY emits no repaint after a resize, and conhost keeps addressing
    /// the screen with absolute coordinates computed against its own
    /// re-anchored layout — see [`Grid::resize_conpty`]. A terminal fed by a
    /// Windows ConPTY should set this so its rows stay where conhost aims.
    ///
    /// [`Grid::resize_conpty`]: crate::grid::Grid::resize_conpty
...
impl<T> Term<T> {
    ...
    pub fn resize<S: Dimensions>(&mut self, size: S) {
        ...
        // ConPTY row semantics apply to the primary screen, whichever grid
        // currently holds it; the alternate screen has no scrollback and its
        // application repaints itself after a resize.
    ...
    /// Give the cell at `line`/`column` a second column after a variation
    /// selector turned its contents into an emoji presentation sequence.
    ///
    /// `input` reserves columns per `char`, so a base whose East Asian Width is
    /// Neutral (`U+2764` ❤, `U+1F5C2` 🗂, `U+26A0` ⚠, …) is already sitting in a
    /// single column by the time its `U+FE0F` arrives — even though the pair
    /// renders as a two-column emoji everywhere else. `UnicodeWidthStr` scores
    /// the *sequence* rather than the char, which is where UTS #51's "emoji
    /// presentation sequences have width 2" rule lives; ask it, and widen the
    /// cell when it disagrees with the single column we handed out.
    ///
    /// Only the cell the cursor just left can be widened: the spacer has to go
    /// into the next column, and that column has to be the cursor's own. A base
    /// left in the last column, or a cursor the application moved between the
    /// two codepoints, keeps the layout it already has.
    fn widen_emoji_presentation(&mut self, line: Line, column: Column) {
        let cell = &self.grid[line][column];
        if cell.flags.contains(Flags::WIDE_CHAR) {
            return;
        }

        let mut sequence = String::with_capacity(8);
        sequence.push(cell.c);
        sequence.extend(cell.zerowidth().into_iter().flatten());
        if UnicodeWidthStr::width(sequence.as_str()) != 2 {
            return;
        }

        let cursor = self.grid.cursor.point;
        if self.grid.cursor.input_needs_wrap || cursor.line != line || cursor.column != column + 1 {
            return;
        }

        self.grid[line][column].flags.insert(Flags::WIDE_CHAR);

        // Spacer into the cursor cell, then step past it — the same two moves
        // `input` makes for a natively-wide glyph.
...
impl<T: EventListener> Handler for Term<T> {
    ...
    fn input(&mut self, c: char) {
        ...
        if width == 0 {
            ...
            // U+FE0F can turn the cell it just landed on into a two-column
            // emoji; the column budget was decided before it arrived.
            if c == '\u{FE0F}' {
                self.widen_emoji_presentation(line, column);
            }
...
mod tests {
    ...
    /// A variation selector can widen the cell it lands on: `U+2764` is one
    /// column on its own, but `U+2764 U+FE0F` (❤️) is a two-column emoji.
    #[test]
    fn emoji_presentation_selector_widens_its_base() {
        let size = TermSize::new(7, 17);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        term.input('\u{2764}');
        term.input('\u{FE0F}');

        let base = &term.grid()[Point::new(Line(0), Column(0))];
        assert_eq!(base.c, '\u{2764}');
        assert!(base.flags.contains(Flags::WIDE_CHAR));
        assert_eq!(base.zerowidth(), Some(&['\u{FE0F}'][..]));

        let spacer = &term.grid()[Point::new(Line(0), Column(1))];
        assert!(spacer.flags.contains(Flags::WIDE_CHAR_SPACER));

        // The next glyph starts past the spacer, not on top of it.
        term.input('x');
        assert_eq!(term.grid()[Point::new(Line(0), Column(2))].c, 'x');
    }

    /// The text presentation selector must not widen anything, and neither must
    /// a selector on a base that has no emoji form at all.
    #[test]
    fn non_widening_variation_selectors_leave_the_column_budget_alone() {
        let size = TermSize::new(7, 17);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        term.input('\u{2764}');
        term.input('\u{FE0E}'); // text presentation
        term.input('A');
        term.input('\u{FE0F}'); // 'A' is not an emoji base

        for column in [Column(0), Column(1)] {
            assert!(!term.grid()[Point::new(Line(0), column)].flags.contains(Flags::WIDE_CHAR));
        }
        assert_eq!(term.grid()[Point::new(Line(0), Column(1))].c, 'A');
    }

    /// An already-wide base (`U+1F60A` 😊) is fully qualified by its selector,
    /// so the sequence stays two columns instead of growing to three.
    #[test]
    fn emoji_presentation_selector_does_not_re_widen_a_wide_base() {
        let size = TermSize::new(7, 17);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        term.input('\u{1F60A}');
        term.input('\u{FE0F}');
        ...
        assert!(term.grid()[Point::new(Line(0), Column(0))].flags.contains(Flags::WIDE_CHAR));
        assert!(
            term.grid()[Point::new(Line(0), Column(1))].flags.contains(Flags::WIDE_CHAR_SPACER)
        );
        assert_eq!(term.grid()[Point::new(Line(0), Column(2))].c, 'x');
    }

    /// No room for the spacer in the last column: the cell keeps the single
    /// column it was given rather than scribbling onto the next line.
    #[test]
    fn emoji_presentation_selector_in_the_last_column_stays_narrow() {
        let size = TermSize::new(3, 17);
        let mut term = Term::new(Config::default(), &size, VoidListener);
        term.input('a');
        term.input('b');
        term.input('\u{2764}');
        ...
        let base = &term.grid()[Point::new(Line(0), Column(2))];
        assert_eq!(base.c, '\u{2764}');
        assert!(!base.flags.contains(Flags::WIDE_CHAR));
        assert_eq!(base.zerowidth(), Some(&['\u{FE0F}'][..]));
    }
