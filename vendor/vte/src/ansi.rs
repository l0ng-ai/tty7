...
impl<T: Timeout> Processor<T> {
    /// In-flight CSI/OSC/UTF-8 and synchronized bytes are part of a checkpoint,
    /// including REP's preceding character. No local Instant crosses the wire.
    #[cfg(all(feature = "serde", feature = "std"))]
    pub fn checkpoint(&self) -> ProcessorCheckpoint {
        ProcessorCheckpoint {
            parser: self.parser.clone(), preceding_char: self.state.preceding_char,
            synchronized: self.state.sync_state.timeout.pending_timeout(),
            sync_buffer: self.state.sync_state.buffer.clone(),
        }
    }

    #[cfg(all(feature = "serde", feature = "std"))]
    pub fn restore_checkpoint(&mut self, checkpoint: ProcessorCheckpoint) -> Result<(), &'static str> {
        if !checkpoint.valid() { return Err("invalid VT parser checkpoint"); }
        self.parser = checkpoint.parser;
        self.state.preceding_char = checkpoint.preceding_char;
        self.state.sync_state.buffer = checkpoint.sync_buffer;
        self.state.sync_state.timeout.clear_timeout();
        if checkpoint.synchronized {
            self.state.sync_state.timeout.set_timeout(Duration::from_millis(150));
        }
        Ok(())
    }
    #[inline]
...
impl ProcessorCheckpoint {
    pub fn valid(&self) -> bool {
        self.parser.valid_checkpoint() && self.sync_buffer.len() < SYNC_BUFFER_SIZE
            && (self.synchronized || self.sync_buffer.is_empty())
    }
}
