...
#[cfg_attr(all(feature = "serde", feature = "std"), derive(serde::Serialize, serde::Deserialize))]
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    fn valid_checkpoint(&self) -> bool {
        self.intermediate_idx <= MAX_INTERMEDIATES
            && self.osc_num_params <= MAX_OSC_PARAMS
            && self.osc_raw.len() <= 2 * 1024 * 1024
            && self.osc_params[..self.osc_num_params].iter()
                .all(|&(start, end)| start <= end && end <= self.osc_raw.len())
            && self.partial_utf8_len <= 3
            && (self.partial_utf8_len == 0
                || str::from_utf8(&self.partial_utf8[..self.partial_utf8_len])
                    .is_err_and(|e| e.valid_up_to() == 0 && e.error_len().is_none()))
            && self.params.valid_checkpoint()
    }
