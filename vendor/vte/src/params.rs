...
impl Params {
    ...
    pub(crate) fn valid_checkpoint(&self) -> bool {
        if self.len > MAX_PARAMS || self.current_subparams as usize > self.len {
            return false;
        }
        let mut i = 0;
        while i < self.len {
            let n = self.subparams[i] as usize;
            if n == 0 || i + n > self.len {
                return false;
            }
            if i + n == self.len && self.current_subparams != 0
                && n != self.current_subparams as usize {
                return false;
            }
            i += n;
        }
